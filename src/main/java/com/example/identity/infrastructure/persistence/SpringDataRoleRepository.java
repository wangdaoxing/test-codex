...
interface SpringDataRoleRepository extends CrudRepository<RoleEntity, UUID> {

    @Query("SELECT * FROM roles WHERE organization_id = :organizationId AND code = :code")
    Optional<RoleEntity> findByCode(@Param("organizationId") UUID organizationId, @Param("code") String code);
